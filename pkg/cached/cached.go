...
func (c *Cached) NodeUnpublishVolume(ctx context.Context, req *csi.NodeUnpublishVolumeRequest) (*csi.NodeUnpublishVolumeResponse, error) {
	...
	logger.Info(ctx, "unpublishing volume", key.VolumeID.Field(volumeID), key.TargetPath.Field(targetPath))
	trace.SpanFromContext(ctx).SetAttributes(key.VolumeID.Attribute(volumeID), key.TargetPath.Attribute(targetPath))
	...
	if !notMounted {
		logger.Info(ctx, "unmounting target path", key.TargetPath.Field(targetPath))
	...
	if err := c.removeSnapshot(ctx, volumeID); err != nil {
		logger.Warn(ctx, "failed to remove snapshot", key.VolumeID.Field(volumeID), zap.Error(err))
	}
...
func (c *Cached) ensurePhysicalVolume(ctx context.Context) error {
	logger.Debug(ctx, "checking physical volume", key.Device.Field(c.LVMDevice))

	err := exec(ctx, "pvdisplay", c.LVMDevice)
	if err == nil {
		logger.Debug(ctx, "physical volume already exists", key.Device.Field(c.LVMDevice))
		return nil
	}
	...
	logger.Info(ctx, "creating physical volume", key.Device.Field(c.LVMDevice))
...
func (c *Cached) ensureVolumeGroup(ctx context.Context) error {
	...
	err := exec(ctx, "vgdisplay", c.lvmVg)
	if err == nil {
		logger.Debug(ctx, "volume group already exists", key.VolumeGroup.Field(c.lvmVg))
		return nil
	}
	...
	logger.Info(ctx, "creating volume group", key.VolumeGroup.Field(c.lvmVg), key.Device.Field(c.LVMDevice))
...
func (c *Cached) ensureThinPool(ctx context.Context) error {
	...
	logger.Debug(ctx, "checking thin pool", key.ThinPool.Field(thinPool))

	err := exec(ctx, "lvdisplay", thinPool)
	if err == nil {
		logger.Debug(ctx, "thin pool already exists", key.ThinPool.Field(thinPool))
		return nil
	}
	...
	logger.Info(ctx, "creating thin pool", key.ThinPool.Field(thinPool))
...
func (c *Cached) ensureBaseVolume(ctx context.Context) error {
	...
	logger.Debug(ctx, "checking base volume", key.LogicalVolume.Field(baseLv), key.Device.Field(baseDevice))

	err := exec(ctx, "lvdisplay", baseLv)
	if err == nil {
		logger.Debug(ctx, "base volume already exists", key.LogicalVolume.Field(baseLv), key.Device.Field(baseDevice))
		return nil
	}
	...
	logger.Info(ctx, "creating base volume", key.LogicalVolume.Field(baseLv), key.Device.Field(baseDevice), key.VirtualSize.Field(c.LVMVirtualSize))
	...
	if err := c.udevSettle(ctx, baseDevice); err != nil {
		logger.Warn(ctx, "udev settle failed for base volume", key.Device.Field(baseDevice), zap.Error(err))
	}
...
func (c *Cached) createSnapshot(ctx context.Context, volumeID string) error {
	...
	logger.Debug(ctx, "checking snapshot", key.LogicalVolume.Field(snapshotPath))

	err := exec(ctx, "lvdisplay", snapshotPath)
	if err == nil {
		logger.Debug(ctx, "snapshot already exists", key.LogicalVolume.Field(snapshotPath))
		return nil
	}
	...
	logger.Info(ctx, "creating snapshot", key.LogicalVolume.Field(snapshotPath))
	...
	if err := c.udevSettle(ctx, devicePath); err != nil {
		...
		logger.Warn(ctx, "udev settle failed for snapshot", key.Device.Field(devicePath), zap.Error(err))
	}
...
func (c *Cached) removeSnapshot(ctx context.Context, volumeID string) error {
	...
	logger.Debug(ctx, "checking snapshot for removal", key.Device.Field(snapshotDevice))

	err := exec(ctx, "lvdisplay", "-q", snapshotDevice)
	if err == nil {
		logger.Debug(ctx, "snapshot already removed", key.Device.Field(snapshotDevice))
		return nil
	}
	...
	logger.Info(ctx, "removing snapshot", key.Device.Field(snapshotDevice))
...
func (c *Cached) udevSettle(ctx context.Context, devPath string) error {
	...
	if err := exec(ctx, "udevadm", "trigger", "--action=add", devPath); err != nil {
		logger.Warn(ctx, "udevadm trigger failed", key.Device.Field(devPath), zap.Error(err))
	...
	if err := exec(settleCtx, "udevadm", "settle", "--exit-if-exists="+devPath); err != nil {
		logger.Warn(ctx, "udev settle failed", key.Device.Field(devPath), zap.Error(err))
		// Fallback to polling
	...
	for {
		...
		case <-ticker.C:
			if _, err := os.Stat(devicePath); err == nil {
				logger.Debug(ctx, "device is available", key.Device.Field(devicePath))
				return nil
			}
