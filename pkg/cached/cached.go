...
// createWritebackCache creates a RAM block device, a volume group to wrap it, and converts the main volume group to use this RAM device as a writeback cache
func (c *Cached) createWritebackCache(ctx context.Context) error {
	if err := c.ensureRamPool(ctx); err != nil {
		return fmt.Errorf("failed to create RAM device: %w", err)
	}

	// invoke lvconvert to create the writeback cache for the thinpool data
