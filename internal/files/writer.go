...
func writeObject(rootDir string, cacheObjectsDir string, reader *db.TarReader, header *tar.Header, existingDirs map[string]bool, hasReflinkSupport bool) error {
	...
	case pb.TarCached:
		...
		if hasReflinkSupport {
			return ReflinkDir(filepath.Join(cacheObjectsDir, hashHex, header.Name), path)
		} else {
...
func WriteTar(finalDir string, cacheObjectsDir string, reader *db.TarReader, packPath *string, matcher *FileMatcher, hasReflinkSupport bool) (uint32, bool, error) {
	var count uint32
	dir := finalDir
	...
	for {
		...
		err = writeObject(dir, cacheObjectsDir, reader, header, existingDirs, hasReflinkSupport)
		if err != nil {
