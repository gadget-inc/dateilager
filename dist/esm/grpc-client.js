...
export class DateiLagerGrpcClient {
    ...
    constructor(options) {
        ...
        this._transport = new GrpcTransport({
            ...
            channelCredentials: credentials.combineChannelCredentials(ChannelCredentials.createSsl(), credentials.createFromMetadataGenerator((_, callback) => {
                tokenFn()
                    .then((token) => {
    ...
    async newProject(project, packPatterns, template) {
        try {
            await trace("dateilager-grpc-client.new-project", {
                attributes: {
    ...
    async deleteProject(project) {
        await trace("dateilager-grpc-client.delete-project", {
            attributes: {
                "dl.project": String(project),
            },
    ...
    async *listObjects(project, path, ignores = [], from, to) {
        ...
        try {
            const call = contextAPI.with(traceAPI.setSpan(parentContext, span), () => this._client.get({
                project,
                fromVersion: from,
                toVersion: to,
                queries: [
    ...
    updateObjects(project) {
        const parentContext = contextAPI.active();
        const span = tracer.startSpan("dateilager-grpc-client.update-objects", {
            attributes: {
                "dl.project": String(project),
            },
        }, parentContext);
